...
class Grille {
    ...
    caseDivs.forEach((div, index) => {
      ...
      img.onclick = (evt) => {
        ...
        if (this.cookieASwap.length === 0) {  //click sur cookie de départ
          ...
          let nb = this.detecterMatch();
          /*if (!nb) {
            this.swapCookies();
            console.log("Aucun match possible car pts potentiellement gagné ="+ nb);
  
          }
          else {*/
            let pointsTotaux = 0;
            let pts;
            let combo = 0; //vérification combo réaparitio à la réaparitions des blocks
            do {
              pts = 0;
              pts += this.detecterMatchAction();
  
              
              if (pts > 0) {
                combo++;
                if (combo) {
                  console.log("COMBO N°" + combo); //vérifie le bon fonctionnement des combos à la réparition des cookies
                  console.log("pts = " + pts);
                }
                this.remplirCasesVides(6);
              }
              pointsTotaux += pts;
              console.log("pts Totaux : " + pointsTotaux);
  
            } while (pts != 0);
  
            combo = 0; //reset du combo
          //}
  ...
  //detecte les matches pour vérifier le swap
  detecterMatch() {
  ...
  // Detecte les matches et intéragie sur les cookies
  detecterMatchAction() {

    let points = 0;
    points += this.detecterMatch3LignesAction();
    points += this.detecterMatch3ColonnesAction();
    return points;
  }

  detecterMatch3LignesAction() {
    let point = 0;
    for (let l = 0; l < this.nbLigne; l++) {
      for (let c = 0; c < this.nbColonne - 2; c++) {
        while (c < this.nbColonne - 2 && this.tabCookies[l][c].type === this.tabCookies[l][c + 1].type && this.tabCookies[l][c].type === this.tabCookies[l][c + 2].type) {
